function format(d) {
    ...
    var productCategories = d.productCategories;
    var productGroup = "";

    if (productCategories) {
        let badges = productCategories
            .filter(item => item.productGroup)
            .map(item => `<span class="badge bg-warning">${item.category.name}</span><br>`)
    ...
    var caseLanguages = d.caseLanguages;
    var languages = "";
    console.log(caseLanguages);

    if (caseLanguages) {
        let badges = caseLanguages
...
$(document).ready(function () {
    ...
    $("#ozelFilter").click(function (e) {
        ...
        tableFunc( from, todate, productCategoryId, doctorId, sectionId);
    });
    ...
    function tableFunc( from = null, todate = null, productCategoryId = null, doctorId = null, sectionId = null) {
            ...
            "columns": [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        var caseCategories = row.caseCategories;
                        var returnData = "";
                        caseCategories = caseCategories.map(function (item) {
                            if (item.productGroup) {
                                returnData += `<i class="fas fa-dot-circle text-success"></i> ` + item.category.categoryParent.name;
                            }
                            else {
                ...
                },
                {
                    ...
                    "searchable": false,
                    render: function (data, type, row, meta) {
                        var caseSections = row.caseSections;
                        var returnData = "";
                        caseSections = caseSections.map(function (item) {
                            returnData += `<i class="fas fa-dot-circle text-dark"></i> ` + item.section.name + "<br>";
                        });
                ...
                },
                {
                    ...
                    "searchable": false,
                    render: function (data, type, row, meta) {
                        var caseDoctors = row.caseDoctors;
                        var returnData = "";
                        caseDoctors = caseDoctors.map(function (item) {
                ...
                },
                {
                    ...
                    "data": null, "render": function (row) {
                        ...
                        var buttons = `<a class='btn btn-warning me-1' href="/CaseUpdate/${id}" data-bs-toggle="tooltip" title="Update Case"><i class="fa-solid fa-pen"></i></a>
                    <button type="button" class="btn btn-danger me-1 deleteButton" data-id="${id}" data-bs-toggle="tooltip" title="Delete Case"><i class="fa-solid fa-trash"></i></button>
                    <a class='btn btn-dark position-relative me-1' href="/CaseSlide/CaseSlideHome/${id}" data-bs-toggle="tooltip" title="Slides">
                            <i class="fa-solid fa-presentation-screen"></i>
                            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                                ${row.caseSlides.length}
                            </span>
                    </a>
    ...
    $('#example tbody').on('click', '.deleteButton', function (e) {
        ...
        iziToast.question({
            timeout: 20000, close: false, overlay: true, displayMode: 'once', id: 'question', zindex: 999, title: 'Hey', message: 'Are you sure about that?', position: 'center',
            buttons: [
