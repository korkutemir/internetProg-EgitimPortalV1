...
- All string literals must be small case. This includes Speaker names and option values.
...
- Every public api Speaker/method should be accompanied by a test.
...
Despite being named 'unit', these are integration tests, making sure public API methods and Speakers work as expected.
...
Every public API method or Speaker should be called at least once in tests. To ensure this, there's a `coverage` command which tracks calls to public API and reports back if some methods/Speakers were not called.
