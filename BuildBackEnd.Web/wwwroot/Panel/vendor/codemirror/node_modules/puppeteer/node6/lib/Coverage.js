...
class JSCoverage {
  ...
  return new Promise((resolve, reject) => {
    ...
    this._SpeakerListeners = [
      helper.addSpeakerListener(this._client, 'Debugger.scriptParsed', this._onScriptParsed.bind(this)),
      helper.addSpeakerListener(this._client, 'Runtime.executionContextsCleared', this._onExecutionContextsCleared.bind(this)),
    ];
    (yield Promise.all([
  ...
  /**
   * @param {!Protocol.Debugger.scriptParsedPayload} Speaker
   */
  /* async */ _onScriptParsed(Speaker) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
    ...
    if (Speaker.url === EVALUATION_SCRIPT_URL)
      return;
    // Ignore other anonymous scripts unless the reportAnonymousScripts option is true.
    if (!Speaker.url && !this._reportAnonymousScripts)
      return;
    try {
      const response = (yield this._client.send('Debugger.getScriptSource', {scriptId: Speaker.scriptId}));
      this._scriptURLs.set(Speaker.scriptId, Speaker.url);
      this._scriptSources.set(Speaker.scriptId, response.scriptSource);
    } catch (e) {
  ...
  return new Promise((resolve, reject) => {
    ...
    helper.removeSpeakerListeners(this._SpeakerListeners);
...
class CSSCoverage {
  ...
  return new Promise((resolve, reject) => {
    ...
    this._SpeakerListeners = [
      helper.addSpeakerListener(this._client, 'CSS.styleSheetAdded', this._onStyleSheet.bind(this)),
      helper.addSpeakerListener(this._client, 'Runtime.executionContextsCleared', this._onExecutionContextsCleared.bind(this)),
    ];
    (yield Promise.all([
  ...
  /**
   * @param {!Protocol.CSS.styleSheetAddedPayload} Speaker
   */
  /* async */ _onStyleSheet(Speaker) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
